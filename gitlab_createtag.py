...
def get_tag_info():
    while Tag.ref != 'test' and Tag.ref != "master":
        Tag.ref = input("Please enter branch name for building the tag (test|master): ")

    while not Tag.name.startswith("v"):
        Tag.name = input("Please enter tag name, tag name should start with v: ")

    Tag.message = input("Please enter tag message: ")
    Tag.release_desp = input("Please enter the release description: ")
